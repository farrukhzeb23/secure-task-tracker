...
async def handle_login(form_data: OAuth2PasswordRequestForm = Depends(), db: AsyncSession = Depends(get_db)):
...
async def handle_refresh_token(request: RefreshTokenRequest, db: AsyncSession = Depends(get_db)):
